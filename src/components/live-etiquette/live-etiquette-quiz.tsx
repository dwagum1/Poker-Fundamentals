...
import {
  LIVE_ETIQUETTE_QUIZ_LEN,
  buildLiveEtiquetteQuestions,
  type LiveEtiquetteQuizQuestion,
} from "@/lib/poker/live-etiquette/quiz";
...
function formatAnswer(_q: LiveEtiquetteQuizQuestion, val: string): string {
  return String(val);
}

export function LiveEtiquetteQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<LiveEtiquetteQuizQuestion[]>([]);
  ...
  const progressLabel = `${index + 1}/${LIVE_ETIQUETTE_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildLiveEtiquetteQuestions(LIVE_ETIQUETTE_QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.liveEtiquette.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: LIVE_ETIQUETTE_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
  ...
  const onPick = (value: string) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= LIVE_ETIQUETTE_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {LIVE_ETIQUETTE_QUIZ_LEN} questions: cards on the table, action in turn,
          string bets, and courtesy toward other players. House rules vary.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{LIVE_ETIQUETTE_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
