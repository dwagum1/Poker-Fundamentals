...
import {
  buildImpliedOddsQuizQuestions,
  IMPLIED_ODDS_QUIZ_LEN,
  type ImpliedOddsQuizQuestion,
} from "@/lib/poker/implied-odds/quiz";
...
function formatAnswer(
  q: ImpliedOddsQuizQuestion,
  val: string | number,
): string {
  if (q.kind === "break_even_pct_implied") return `${val}%`;
  return String(val);
}

export function ImpliedOddsQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<ImpliedOddsQuizQuestion[]>([]);
  ...
  const progressLabel = `${index + 1}/${IMPLIED_ODDS_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildImpliedOddsQuizQuestions(IMPLIED_ODDS_QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.impliedOdds.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: IMPLIED_ODDS_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
          impliedOdds: {
            ...
            quizHistory: appendQuizHistory(p.impliedOdds.quizHistory, attempt),
          },
        };
  ...
  const onPick = (value: string | number) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= IMPLIED_ODDS_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {IMPLIED_ODDS_QUIZ_LEN} questions: definitions, when implied odds
          matter, caveats, reverse implied odds, and integer break-even equity
          with a simplified “extra pot when you win” snapshot.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{IMPLIED_ODDS_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
  ...
  const isNumeric = current.kind === "break_even_pct_implied";
