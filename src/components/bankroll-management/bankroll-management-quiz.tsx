...
import {
  BANKROLL_MANAGEMENT_QUIZ_LEN,
  buildBankrollManagementQuizQuestions,
  type BankrollManagementQuizQuestion,
} from "@/lib/poker/bankroll-management/quiz";
...
function formatAnswer(
  q: BankrollManagementQuizQuestion,
  val: string | number,
): string {
  if (q.kind === "buyin_count") return String(val);
  return String(val);
}

export function BankrollManagementQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<BankrollManagementQuizQuestion[]>(
    [],
  );
  ...
  const progressLabel = `${index + 1}/${BANKROLL_MANAGEMENT_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(
      buildBankrollManagementQuizQuestions(BANKROLL_MANAGEMENT_QUIZ_LEN),
    );
    setIndex(0);
    setScore(0);
    setPicked(null);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.bankrollManagement.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: BANKROLL_MANAGEMENT_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
          bankrollManagement: {
            ...
            quizHistory: appendQuizHistory(
              p.bankrollManagement.quizHistory,
              attempt,
            ),
          },
        };
  ...
  const onPick = (value: string | number) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= BANKROLL_MANAGEMENT_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {BANKROLL_MANAGEMENT_QUIZ_LEN} questions: variance, bankroll basics,
          buy-in counting, and discipline ideas. Not financial advice.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{BANKROLL_MANAGEMENT_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
  ...
  const isNumeric = current.kind === "buyin_count";
    ...
    <div className="space-y-4">
      ...
      >
        {current.options.map((opt, optIdx) => {
          ...
          const label = isNumeric ? String(opt) : opt;
