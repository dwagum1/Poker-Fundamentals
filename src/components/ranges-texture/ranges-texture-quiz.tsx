...
import {
  RANGES_TEXTURE_QUIZ_LEN,
  buildRangesTextureQuizQuestions,
  type RangesTextureQuizQuestion,
} from "@/lib/poker/ranges-texture/quiz";
...
export function RangesTextureQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<RangesTextureQuizQuestion[]>([]);
  ...
  const progressLabel = `${index + 1}/${RANGES_TEXTURE_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(
      buildRangesTextureQuizQuestions(RANGES_TEXTURE_QUIZ_LEN),
    );
    setIndex(0);
    setScore(0);
    setPicked(null);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.rangesAndTexture.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: RANGES_TEXTURE_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
          rangesAndTexture: {
            ...
            quizHistory: appendQuizHistory(
              p.rangesAndTexture.quizHistory,
              attempt,
            ),
          },
        };
  ...
  const onPick = (value: string) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= RANGES_TEXTURE_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {RANGES_TEXTURE_QUIZ_LEN} questions: ranges, position, dry/wet and
          paired boards, and simple who-benefits heuristics. NLHE fundamentals
          only.
        </p>
        <button
          type="button"
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{RANGES_TEXTURE_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Link
            href="/ranges-and-texture/history"
