...
import {
  buildPositionQuizQuestions,
  seatLabel,
  type PositionQuizQuestion,
} from "@/lib/poker/position/quiz";
...
import type { SeatId } from "@/lib/poker/position/nine-max";
...
export function PositionQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<PositionQuizQuestion[]>([]);
  ...
  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildPositionQuizQuestions(QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
      ...
      updateProgress((p) => {
        ...
        const next = {
          ...
          position: {
            ...
            quizHistory: appendQuizHistory(p.position.quizHistory, attempt),
          },
        };
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {QUIZ_LEN} questions on acting order and heads-up position. Assumes all
          nine players are dealt in unless the prompt says heads-up.
        </p>
        <button
          type="button"
          onClick={start}
