...
import {
  buildMultiwayQuizQuestions,
  type MultiwayQuizQuestion,
} from "@/lib/poker/position/multiway-quiz";
import { seatLabel, type SeatId } from "@/lib/poker/position/nine-max";
...
export function MultiwayPositionQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<MultiwayQuizQuestion[]>([]);
  const [index, setIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [picked, setPicked] = useState<SeatId | null>(null);
  ...
  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildMultiwayQuizQuestions(QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.position.multiwayQuiz;
        ...
        const next = {
          ...
          position: {
            ...
            multiwayQuizHistory: appendQuizHistory(
              p.position.multiwayQuizHistory,
              attempt,
            ),
          },
        };
  ...
  const onPick = (seat: SeatId) => {
    if (!current || showResult) return;
    setPicked(seat);
    setShowResult(true);
    const correct = seat === current.answer;
    logRef.current.push({
      questionId: current.id,
      prompt: current.prompt,
      userAnswer: seatLabel(seat),
      correctAnswer: seatLabel(current.answer),
      correct,
    });
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-50">
          Multiway position quiz
        </h2>
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {QUIZ_LEN} questions: first and last to act with three or four active
          players, plus pairwise “who is IP” between Hero and Villain.
        </p>
        <button
          type="button"
          onClick={start}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-2">
        {current.options.map((seat) => {
          const isAnswer = seat === current.answer;
          const isPickedSeat = picked === seat;
          const showCorrect = showResult && isAnswer;
          const showWrong = showResult && isPickedSeat && !isAnswer;
          return (
            <button
              key={seat}
              type="button"
