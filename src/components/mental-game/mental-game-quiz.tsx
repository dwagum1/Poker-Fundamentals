...
import {
  MENTAL_GAME_QUIZ_LEN,
  buildMentalGameQuestions,
  type MentalGameQuizQuestion,
} from "@/lib/poker/mental-game/quiz";
...
function formatAnswer(_q: MentalGameQuizQuestion, val: string): string {
  return String(val);
}

export function MentalGameQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<MentalGameQuizQuestion[]>([]);
  ...
  const progressLabel = `${index + 1}/${MENTAL_GAME_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildMentalGameQuestions(MENTAL_GAME_QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.mentalGame.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: MENTAL_GAME_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
          mentalGame: {
            ...
            quizHistory: appendQuizHistory(p.mentalGame.quizHistory, attempt),
          },
        };
  ...
  const onPick = (value: string) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= MENTAL_GAME_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {MENTAL_GAME_QUIZ_LEN} questions: tilt, focus, session habits, and how
          mindset differs from bankroll math. Educational only.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{MENTAL_GAME_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
