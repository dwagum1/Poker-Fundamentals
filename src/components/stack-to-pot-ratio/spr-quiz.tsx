...
import {
  buildSprQuizQuestions,
  type SprQuizQuestion,
} from "@/lib/poker/stack-to-pot-ratio/quiz";
...
function formatSprAnswer(q: SprQuizQuestion, val: string | number): string {
  if (q.kind === "concept") return String(val);
  return `${val}`;
}

function numericSprEqual(a: number, b: number): boolean {
  return Math.round(a * 10) === Math.round(b * 10);
}

export function SprQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<SprQuizQuestion[]>([]);
  ...
  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildSprQuizQuestions(QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.stackToPotRatio.quiz;
  ...
  const onPick = (value: string | number) => {
    ...
    if (q.kind === "concept") {
      ...
      correct = numericSprEqual(value as number, q.answer);
    }
    logRef.current.push({
      questionId: q.id,
      prompt: q.prompt,
      userAnswer: formatSprAnswer(q, value),
      correctAnswer: formatSprAnswer(q, q.answer),
      correct,
    });
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {QUIZ_LEN} questions: SPR from effective stack and pot, SPR after a
          flop-style bet and call, and short commitment concepts. Answers use SPR
          rounded to one decimal where numbers are asked.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {current.options.map((opt) => {
          const showC = showResult && numericSprEqual(opt, current.answer);
          const showW =
            showResult &&
            picked !== null &&
            numericSprEqual(opt as number, picked as number) &&
            !numericSprEqual(opt, current.answer);
          return (
            <button
              key={opt}
              type="button"
