...
import {
  buildPotOddsQuizQuestions,
  type PotOddsQuizQuestion,
} from "@/lib/poker/pot-odds/quiz";
...
function formatPotOddsAnswer(
  q: PotOddsQuizQuestion,
  val: string | number,
): string {
  if (q.kind === "ratio") return String(val);
  if (q.kind === "break_even_pct" || q.kind === "rule24") return `${val}%`;
  return String(val);
}
...
export function PotOddsQuiz() {
  ...
  const [questions, setQuestions] = useState<PotOddsQuizQuestion[]>([]);
  ...
  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildPotOddsQuizQuestions(QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
      ...
      updateProgress((p) => {
        ...
        const next = {
          ...
          potOdds: {
            ...
            quizHistory: appendQuizHistory(p.potOdds.quizHistory, attempt),
          },
        };
  ...
  const onPick = (value: string | number) => {
    ...
    } else if (q.kind === "break_even_pct" || q.kind === "rule24") {
      correct = value === q.answer;
    } else {
      correct = value === q.answer;
    }
    logRef.current.push({
      questionId: q.id,
      prompt: q.prompt,
      userAnswer: formatPotOddsAnswer(q, value),
      correctAnswer: formatPotOddsAnswer(q, q.answer),
      correct,
    });
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {QUIZ_LEN} questions: pot odds ratios, break-even equity, rule of 2/4,
          and call-or-fold decisions. Integer chips; full call (NLHE).
        </p>
        <button
          type="button"
          onClick={start}
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-2">
        {current.options.map((opt) => {
          const val: string | number =
            current.kind === "ratio" ? opt : (opt as number);
          const label = current.kind === "ratio" ? opt : `${opt}%`;
