...
import { buildEvQuizQuestions, type EvQuizQuestion } from "@/lib/poker/expected-value/quiz";
...
function formatEvAnswer(q: EvQuizQuestion, val: string | number): string {
  if (q.kind === "ev_call" || q.kind === "bluff_ev" || q.kind === "bluff_breakeven") {
    return `$${val}`;
  }
  if (q.kind === "ev_call_sign") return val === "yes" ? "Yes" : "No";
  return String(val);
}

export function ExpectedValueQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<EvQuizQuestion[]>([]);
  ...
  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(buildEvQuizQuestions(QUIZ_LEN));
    setIndex(0);
    setScore(0);
    setPicked(null);
    setShowResult(false);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.expectedValue.quiz;
        ...
        const next = {
          ...
          expectedValue: {
            ...
            quizHistory: appendQuizHistory(p.expectedValue.quizHistory, attempt),
          },
        };
  ...
  const onPick = (value: string | number) => {
    ...
    logRef.current.push({
      questionId: q.id,
      prompt: q.prompt,
      userAnswer: formatEvAnswer(q, value),
      correctAnswer: formatEvAnswer(q, q.answer),
      correct,
    });
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {QUIZ_LEN} questions: EV of calling, pure-bluff EV, break-even fold
          frequency, and short conceptual checks. Integer chips; teaching
          formulas are stated in the prompts.
        </p>
        <button
          type="button"
          onClick={start}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {current.options.map((opt) => {
            const showC = showResult && current.answer === opt;
            const showW =
              showResult && picked === opt && current.answer !== opt;
            return (
              <button
                key={opt}
                type="button"
                ...
                onClick={() => onPick(opt)}
                className={[
                  "rounded-xl border px-4 py-4 text-sm font-semibold capitalize transition",
                  showC
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2">
          {current.options.map((opt) => {
            ...
            const showW = showResult && picked === opt && opt !== current.answer;
            return (
              <button
                key={opt}
                type="button"
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-2">
        {current.options.map((opt) => {
          ...
          const label =
            current.kind === "bluff_breakeven" ? `${opt}%` : `$${opt}`;
