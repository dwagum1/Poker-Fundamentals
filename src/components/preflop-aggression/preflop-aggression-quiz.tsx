...
import {
  PREFLOP_AGGRESSION_QUIZ_LEN,
  buildPreflopAggressionQuizQuestions,
  type PreflopAggressionQuizQuestion,
} from "@/lib/poker/preflop-aggression/quiz";
...
export function PreflopAggressionQuiz() {
  const { updateProgress } = useApp();
  const [phase, setPhase] = useState<"intro" | "active" | "done">("intro");
  const [questions, setQuestions] = useState<PreflopAggressionQuizQuestion[]>(
    [],
  );
  ...
  const progressLabel = `${index + 1}/${PREFLOP_AGGRESSION_QUIZ_LEN}`;

  const start = useCallback(() => {
    logRef.current = [];
    setQuestions(
      buildPreflopAggressionQuizQuestions(PREFLOP_AGGRESSION_QUIZ_LEN),
    );
    setIndex(0);
    setScore(0);
    setPicked(null);
  ...
  const finish = useCallback(
    (finalScore: number) => {
      ...
      setLastWrong(wrong);
      updateProgress((p) => {
        const prev = p.preflopAggression.quiz;
        const best = Math.max(prev.bestScore, finalScore);
        const attempt = {
          completedAt: new Date().toISOString(),
          score: finalScore,
          total: PREFLOP_AGGRESSION_QUIZ_LEN,
          wrong,
        };
        const next = {
          ...p,
          preflopAggression: {
            ...
            quizHistory: appendQuizHistory(
              p.preflopAggression.quizHistory,
              attempt,
            ),
          },
        };
  ...
  const onPick = (value: string) => {
    ...
    window.setTimeout(() => {
      const atEnd = index + 1 >= PREFLOP_AGGRESSION_QUIZ_LEN;
      if (atEnd) {
        setScore(nextScore);
        finish(nextScore);
        return;
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
          {PREFLOP_AGGRESSION_QUIZ_LEN} questions: open, iso, 3-bet, 4-bet, and
          how position and ranges connect. NLHE beginner level only.
        </p>
        <button
          type="button"
          onClick={start}
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
        ...
        <p className="mt-2 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">
          Score: {score}/{PREFLOP_AGGRESSION_QUIZ_LEN}
        </p>
        {lastWrong.length > 0 && (
